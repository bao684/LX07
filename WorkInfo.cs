...
namespace DAL
{
    public class WorkInfo
    {
        ...
        public IEnumerable<Model.WorkInfo> GetRecommend()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select * from workinfo where workVerify='审核通过'and recommend='是' order by recommend desc limit 6 ";
                return cn.Query<Model.WorkInfo>(sql);
            }
        }
        public int GetCount()
        ...
        public IEnumerable<Model.WorkInfoNo> GetMyPage(Model.WorkPage page)
        {
            using (IDbConnection cn=new MySqlConnection(cns))
            {
                ...
                sql += "select * from a where num bteween (@pageIndex-1)*@pageSize+1 and @pageIndex*@pageSize";
                return cn.Query<Model.WorkInfoNo>(sql, page);
            }
        }
        ...
        public object GetMyPage(WorkMyPage page)
        {
        ...
        public int UpdateImg(Model.WorkInfo workinfo)
        {
            using(IDbConnection cn=new MySqlConnection(cns))
            {
                string sql = "update workinfo set workPicture=@wordPicture where workId=@workId";
        ...
        public int UpdateVerify(Model.WorkInfo workinfo)
        {
            using(IDbConnection cn=new MySqlConnection(cns))
            {
                string sql = "update workinfo set workVerify=@workVerity where workId=@workId";
                return cn.Execute(sql, workinfo);
            }
        }
